    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
        <div class="p-6 text-gray-900">
            <div class="flex justify-between mb-6">
                <h3 class="text-lg font-bold">Daftar Gejala</h3>
                <a href="{{ route('symptoms.create') }}" class="bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded text-sm">
                    Tambah Gejala
                </a>
            </div>
                        ...
                        @forelse ($symptoms as $symptom)
                            <tr class="hover:bg-gray-50">
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">{{ $symptom->symptom_code }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <div class="flex items-center justify-center gap-3">
                                        <a href="{{ route('symptoms.edit', $symptom->symptom_code) }}" class="text-blue-600 hover:text-blue-900 inline-flex items-center" title="Ubah">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                              <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                            </svg>
                                        </a>
                                        <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-deletion'); $dispatch('set-delete-url', '{{ route('symptoms.destroy', $symptom->symptom_code) }}')" class="text-red-600 hover:text-red-900 inline-flex items-center" title="Hapus">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    <x-modal name="confirm-deletion" focusable>
        <form method="post" x-data="{ action: '' }" x-on:set-delete-url.window="action = $event.detail" x-bind:action="action" class="p-6">
            ...
            <p class="mt-1 text-sm text-gray-600">
                Menghapus Gejala ini akan menghapus semua Aturan yang terkait dengan Gejala tersebut.
            </p>
